...
import cz.maxtechnik.opm.client.widget.*;
...
public class RecipeEditor extends Screen{
	...
	public boolean mouseClicked(double mouseX,double mouseY,int button){
		...
		if(button==0&&renderer.hit(bmx,bmy,saveBtnX,saveBtnY,90,16)){
			save();
			return true;
		}
	...
	private boolean handleEditorClicks(int mx,int scrolledY){
		...
		if(type==StationType.MECH_CRAFTING){
			...
			if(renderer.hit(localMx,localMy,bx,by,bw,bh)){
				shiftMechGrid(0,-1);
				return true;
			}
			if(renderer.hit(localMx,localMy,bx+bw+2,by,bw,bh)){
				shiftMechGrid(0,1);
				return true;
			}
			if(renderer.hit(localMx,localMy,bx,by+bh+2,bw,bh)){
				shiftMechGrid(-1,0);
				return true;
			}
			if(renderer.hit(localMx,localMy,bx+bw+2,by+bh+2,bw,bh)){
				shiftMechGrid(1,0);
				return true;
			}
		}
