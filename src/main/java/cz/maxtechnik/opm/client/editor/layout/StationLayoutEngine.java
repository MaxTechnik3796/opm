			...
			if(crushOuts!=null){
				for(int i=0;i<outG.getTotalSlots()&&i<crushOuts.size();i++){
					...
					if(outG.getSpec().hasChance()){
						int chX=cpx+28;
						if(UiKit.hit(mx,my,chX,cpy-2,UiKit.MINI_SPIN,UiKit.MINI_SPIN)){
							co.chance=Math.min(1f,co.chance+0.05f);
							return true;
						}
						if(UiKit.hit(mx,my,chX,cpy+7,UiKit.MINI_SPIN,UiKit.MINI_SPIN)){
		...
		if(inG!=null&&inG.getSpec().hasCount()){
			...
			for(int i=0;i<inG.getTotalSlots()&&i<inList.size();i++){
				...
				if(UiKit.hit(mx,my,spinX,cpy-2,UiKit.MINI_SPIN,UiKit.MINI_SPIN)){
					if(!s.isEmpty()) s.setCount(Math.min(64,s.getCount()+1));
					return true;
				}
				if(UiKit.hit(mx,my,spinX,cpy+7,UiKit.MINI_SPIN,UiKit.MINI_SPIN)){
					if(!s.isEmpty()) s.setCount(Math.max(1,s.getCount()-1));
					return true;
				}
			}
		}
		...
		if(inG!=null&&outG!=null){
			...
			if(crushOuts!=null){
				...
				for(int i=0;i<outG.getTotalSlots()&&i<crushOuts.size();i++){
					...
					if(outG.getSpec().hasChance()&&UiKit.hit(mx,my,cpx+38,cpy+1,30,12)){
						callback.edit(prefix+"chance",cpx+42,cpy+3,26,String.valueOf((int)(co.chance*100)),i);
						return true;
					}
				}
			}else{
				...
				if(UiKit.hit(mx,my,cpx,cpy+2,14,12)){
					callback.edit(type.name().toLowerCase(Locale.ROOT)+"_count",cpx,cpy+2,20,String.valueOf(getResultCount(d,type)),-1);
					return true;
				}
			}
		}
