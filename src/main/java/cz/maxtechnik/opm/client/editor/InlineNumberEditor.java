	...
	public void apply(RecipeEditorData data,StationType station){
		...
		try{
			...
			if(fieldName.endsWith("_out_count")){
				...
				case "fluid_mix_in" -> {
					if(fieldIndex>=0&&fieldIndex<data.mixFluidIng.size()) data.mixFluidIng.get(fieldIndex).amount=Math.clamp(parsedInt,1,1000);
				}
				case "fluid_mix_out" -> {
					if(fieldIndex>=0&&fieldIndex<data.mixFluidOuts.size()) data.mixFluidOuts.get(fieldIndex).amount=Math.clamp(parsedInt,1,1000);
				}
