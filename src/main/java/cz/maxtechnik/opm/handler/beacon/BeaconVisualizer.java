...
public class BeaconVisualizer{
	...
	private static int getCustomEffectColor(Holder<MobEffect> effectHolder){
		...
		if(effect.equals(MobEffects.MOVEMENT_SPEED.value())) return 0x00D0FF;
		else if(effect.equals(MobEffects.DIG_SPEED.value())) return 0xFFD700;
		else if(effect.equals(MobEffects.DAMAGE_RESISTANCE.value())) return 0x7F8C8D;
		else if(effect.equals(MobEffects.JUMP.value())) return 0x2ECC71;
		else if(effect.equals(MobEffects.DAMAGE_BOOST.value())) return 0xFF0000;
		else if(effect.equals(MobEffects.REGENERATION.value())) return 0xFF69B4;
		return -1;
	}
	@SubscribeEvent
	public static void onRightClickBlock(PlayerInteractEvent.RightClickBlock event){
		if(event.getLevel().isClientSide()) if(event.getLevel().getBlockState(event.getPos()).is(Blocks.BEACON)) lastInteractedBeacon=event.getPos();
	}
	@SubscribeEvent
	public static void onRenderLevelStage(RenderLevelStageEvent event){
		...
		if(mc.screen instanceof BeaconScreen&&lastInteractedBeacon!=null){
	...
	private static void renderBeacons(PoseStack poseStack,Vec3 cameraPos,VertexConsumer consumer,boolean isZone){
		for(BeaconBlockEntity beacon: BEACONS){
			...
			int colorInt=0xadFFF0;
			Holder<MobEffect> cachedEffect=CACHED_EFFECTS.get(pos);
